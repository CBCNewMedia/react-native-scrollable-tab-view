...
:warning: **NOTE**: We removed the `react-native-pager-view` dependency from `package.json` because it was conflicting with our existing version of `react-native-pager-view`. If you install this package, you must also have `react-native-pager-view` installed.
